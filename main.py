...
# User with ID 1 in database is Admin:
# email: admin@email.com
# password: adminpass
# Dummy user with ID 2 in database is Lewis Hamilton:
# email: lewis@email.com
# password: lewispass

load_dotenv()

# For secrets stored in .env
...
class User(UserMixin, db.Model):
    ...
    # Create Parent relationship of user to the blog post
    # This will act like a List of BlogPost objects attached to each User.
    # The "author" refers to the author property in the BlogPost class.
    posts = relationship("BlogPost", back_populates="author")
    # Other Parent relationship of user to the comments
...
class BlogPost(db.Model):
    ...
    # Create Child relationship of the blog post to the user
    # Create Foreign Key, "users.id" the users refers to the tablename of User.
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    # Create reference to the User object, the "posts" refers to the posts property in the User class.
    author = relationship("User", back_populates="posts")
    # Create Parent relationship of the blog post to the comments
...
class Comment(db.Model):
    ...
    # Create Child relationship of the comments to the user
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    comment_author = relationship("User", back_populates="comments")
    # Create Child relationship of the comments to the blog post
...
def contact():
    if request.method == 'POST':
        data = request.form
        send_email(data["name"], data["email"], data["phone_num"], data["message"])
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, current_year=CURRENT_YEAR, is_edit=True)
